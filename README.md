...
It exists for one thing a browser cannot do: **play audio with the screen off.**
The web app is better on a desktop and stays the primary client; this is what
gets used while walking around the house with a phone in a pocket.
...
<p align="center">
  ...
  <img src="docs/screenshots/subtitles.png" width="30%" alt="The player's settings sheet: subtitles, Vietnamese voice, autoplay" />
</p>
<p align="center">
  ...
  <img src="docs/screenshots/narration.png" width="30%" alt="The Vietnamese voice: its two levels and the name the speech service uses" />
</p>

## What it does

- **Home**, with the server's topic chips, *Continue watching*, and **Missed** —
  what the channels you follow posted in the last day that nobody has watched.
...
- **Vietnamese narration**: the server reads the captions, translates them and
  ...
  audio while a line is spoken. Its two levels — how loud the voice is, and how
  far the video drops under it — are the viewer's, and they are separate
  numbers rather than one chained through the other.
- **Background audio**, a lock-screen entry with artwork and a working scrubber,
  and a miniplayer that crosses every screen.
- **Search**, which reaches the library *and* YouTube — opening an upstream
  result writes its catalogue row first, so the player never lands on nothing.
...
- **Streams only.** Nothing is downloaded to the phone. The accepted consequence
  is stated plainly: background audio works around the house, not on a commute.
- The server's address is typed by hand on first run, like Home Assistant —
  mDNS dies on ordinary routers, and the Mac's address is DHCP.
...
That is the verification command, and it is short of `./gradlew check` for a
reason worth stating: `check` links a Kotlin/Native test binary, which needs
full Xcode. What it therefore does not cover is iOS linking and
`iosSimulatorArm64Test` — recorded as uncovered rather than quietly dropped.
...
proven to fail** — an `io.ktor` import in `domain`, and a hardcoded `"Try again"`
in a screen, each turn the build red.
...
- **Nothing in `domain` or a ViewModel is nullable; DTOs are.** The wire really
  ...
  than implied.
- **No Compose UI tests.** Previews cover what a screen looks like — including
  the states that are hard to reach on a device — and ViewModel tests cover what
  it does.
...
The glass is the app's own material rather than the platform's: panes sample
what is behind them and refract at the rim, and the controls over the video are
drawn by SwiftUI on iOS 26, because a video is the one backdrop Compose cannot
sample.
